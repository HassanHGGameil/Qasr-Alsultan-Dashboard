...
interface ListGroupActionProps {
  items: CategoryColumnType[];
  onReorder: (updateData: { id: string; position: number }[]) => void;
  onEdit?: (id: string) => void;
  categoryProducts?: (id: string) => void;
  onDelete?: (id: string) => void;
}

const CategoryListGroupAction = ({
  items,
  onReorder,
  onEdit,
  categoryProducts,
  ...
  const [faqCode, setFaqCode] = useState<CategoryColumnType[]>([]);
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      const reordered = Array.from(faqCode);
      const [moved] = reordered.splice(source.index, 1);
      reordered.splice(destination.index, 0, moved);
      setFaqCode(reordered);
  ...
  const filteredQrCodes = useMemo(() => {
    if (!search.trim()) return faqCode;
    const q = search.toLowerCase();
    return faqCode.filter(
      (item) =>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1  group-hover:opacity-100 transition-opacity duration-200">
                            {categoryProducts && (
                              ...
                              >
                                <HiOutlineRectangleStack className="w-4 h-4 text-foreground" />
                              </Button>
                            )}
...
export default CategoryListGroupAction;
