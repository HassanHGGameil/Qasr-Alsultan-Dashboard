...
import {  MainSection, SectionImages } from "@prisma/client";
import { MainSectionSchema } from "@/validations/sections/mainSection";

type HeroFormValues = z.infer<typeof MainSectionSchema>;
...
const MainSectionForm: React.FC<HeroFormProps> = ({ initialData }) => {
  ...
  const title = initialData ? "Edit MainSection" : "Create MainSection";
  const description = initialData
    ? "Edit the selected MainSection details."
    : "Add a new MainSection entry.";
  const toastMessage = initialData
    ? "MainSection updated successfully."
    : "MainSection created successfully.";
  const action = initialData ? "Save Changes" : "Create MainSection";
  ...
  const form = useForm<HeroFormValues>({
    ...
    defaultValues: initialData
      ? {
          ...
          sectionImages: initialData.sectionImages.map((img) => ({
            url: img.url,
            id: img.id,
            mainSectionId: img.mainSectionId,
          })),
        }
      : {
          titleEn: "",
  ...
  const onSubmit = async (data: HeroFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`${DOMAIN}/api/sections/mainSection/${params.id}`, data);
      } else {
        await axios.post(`${DOMAIN}/api/sections/mainSection`, data);
      }

      toast.success(toastMessage);
      router.push(`/dashboard/sections/mainSection`);
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`${DOMAIN}/api/sections/mainSection/${params.id}`);
